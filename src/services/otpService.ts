...
export const otpService = {
  ...
  async verifyOtpByRequestId(payload: { phone: string; requestId: string; purpose: OtpPurpose }) { const phone = normalizePhone(payload.phone); const purpose = purposeToDb[payload.purpose]; const now = new Date(); const otp = await prisma.phoneOtp.findFirst({ where: { phone, purpose, provider: 'PLUSOFON', providerRequestId: payload.requestId }, orderBy: { createdAt: 'desc' } }); if (!otp) throw new Error('OTP_INVALID'); if (otp.expiresAt <= now) throw new Error('OTP_EXPIRED'); if (!otp.consumedAt) { const statusResult = await this.getOtpStatusByRequestId(payload.requestId); if (statusResult.status !== 'verified') throw new Error('OTP_INVALID'); await this.markOtpVerifiedByProviderRequestId({ requestId: payload.requestId, status: 'verified' }); } return { phone }; },
